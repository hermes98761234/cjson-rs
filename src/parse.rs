...
pub fn from_reader<R: std::io::Read>(mut r: R) -> Result<Value, Error> {
    ...
    r.read_to_string(&mut s)
        .map_err(|_e| Error::new(ErrorKind::InvalidUtf8, 0, 1, 1))?;
    parse(&s)
}
...
impl<'a> Parser<'a> {
    ...
    fn expect_char(&mut self, expected: char) -> Result<(), Error> {
        ...
        match self.peek() {
            ...
            Some(c) => Err(self.err(ErrorKind::UnexpectedToken {
            ...
            match self.peek() {
                ...
                Some('\\') => {
                    ...
                    match self.peek() {
                        ...
                        Some(_c) => return Err(self.err(ErrorKind::InvalidStringEscape)),
                        None => return Err(self.err(ErrorKind::UnexpectedEndOfInput)),
                    }
                }
                Some(c) => {
    ...
    fn parse_unicode_escape(&mut self) -> Result<char, Error> {
        let hex: String = (0..4)
            ...
            .collect();
        if hex.len() != 4 {
            return Err(self.err(ErrorKind::InvalidUnicodeSurrogate));
        }
        let code = u32::from_str_radix(&hex, 16)
            .map_err(|_| self.err(ErrorKind::InvalidUnicodeSurrogate))?;
